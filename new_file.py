...
def load_chat(username):
    chats = ChatMessage.query.filter_by(username=username)\
...
def get_summary(username):
    s = ChatSummary.query.filter_by(username=username).first()
    return s.summary if s else ""


def update_summary(username):
    messages = ChatMessage.query.filter_by(username=username)\
...
Focus on user preferences, facts, and goals.
...
"""
    ...
    try:
        ...
        summary_text = res["message"]["content"]

        existing = ChatSummary.query.filter_by(username=username).first()

        if existing:
            existing.summary = summary_text
            existing.updated_at = datetime.utcnow()
        else:
            db.session.add(ChatSummary(username=username, summary=summary_text))
...
def chat():
    ...
    # MEMORY (UPDATED)
    # ======================
    history = ChatMessage.query.filter_by(username=username)\
        .order_by(ChatMessage.timestamp.desc())\
        .limit(15)\
        .all()
...
def clear_chat():
    ...
    ChatSummary.query.filter_by(username=username).delete()
